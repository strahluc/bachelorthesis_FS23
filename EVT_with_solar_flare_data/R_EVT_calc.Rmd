...
```{r}
#df_test <- read.csv2("df_extreme.csv", header = TRUE, sep = ',')

df_sf <- read.csv2("R_df_solarflare.csv", header = FALSE)
solarflare = as.numeric(as.list(df_sf$V1))
head(solarflare)

df_zifp1 <- read.csv2("R_df_solarflare_zipf_1.csv", header = FALSE)
zipf_1 = as.numeric(as.list(df_zifp1$V1))
head(zipf_1)

df_zifp2 <- read.csv2("R_df_solarflare_zipf_2.csv", header = FALSE)
zipf_2 = as.numeric(as.list(df_zifp2$V1))
head(zipf_2)

df_zifp3 <- read.csv2("R_df_solarflare_zipf_3.csv", header = FALSE)
...
quantile(solarflare, probs = c(0.96,0.976, 0.991)) # liefert die Werte zu den Perzentielen 96, 97.6 und 99.1
...
- Das letzte signifikante Xi ist beim Threshold 0.00032, alles darüber ist nicht signifikant
...
# erster Loop mit grösseren Abständen, um etwa den richtigen Ort zu finden
...
for (i in thresholds_3) {
  ...
  print("ests:")
  print(gpd(solarflare, i)$par.ests[1])
  print("ses:")
  print(gpd(solarflare, i)$par.ses[1])
  print("------------------------------")
}
```
...
```{r}
gpd.sfall(tailplot(fit), 0.999867145, ci.p = 0.99)
```
